...
interface GuideBottomNavigationProps {
  className?: string;
}

const GuideBottomNavigation = () => {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 bg-white border-t border-gray-200 flex justify-around py-2 z-30 bottom-navigation">
      {navItems.map((item) => (
        ...
        >
          <div className={isActive(item.path) ? "text-[#DC143C]" : "text-gray-600"}>
            {item.icon}
          </div>
          <span className={`text-xs mt-1 ${isActive(item.path) ? "text-[#DC143C]" : "text-gray-600"}`}>
            {item.name}
          </span>
        </button>
      ))}
